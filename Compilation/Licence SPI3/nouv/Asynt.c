...
//Grammaire algébrique LL(1):
	...
	//		Emprunt->"<" NLivre Nom			  Livre répertorié et Nom en TDS
	//		Reservation->"?" NLivre Nom		  Livre répertorié et Nom en TDS
	//		Achat->Prix Livre    			  réel>0 et Livre non encore répertorié
	//		Restitution->">" NLivre Nom		  Livre répertorié et Nom en TDS 
	//		Radiation->Nom      			  Nom en TDS
	//		Autre->Entier Suite				  Entier est une durée>0 pr une Interdiction et une date pr une inscription
	//		NLivre->LiLivre||unLivre
	//		LiLivre->"(" DesLivre ")"
	//		DesLivre->D1||D2
	//		D1->unLivre
	//		D2->unLivre DesLivre	
...
int bAchaT(int iDebut,int *piFin){
	//Achat->Prix Livre
	int bAjouter,bSucces,iFin,iMilieu,nItem,suit,res,over;
	char *sLivre;
	float p;
		...
		*piFin=(bSucces)? iFin:iDebut;
		...
		if (bSucces && mAsyntAnalyseR==mAsyntGenerer){
			...
			if(n==1){
				...
				res=bSeparateuR(iFin,&suit)&&bFinFichieR(suit,&over);
			...
			if(n==2){
				int cour=adr;
				for(int i=0;i<strlen(sLivre);i=i+4){
					printf("EMP ");
					//printf("EMP %c%c%c%c\n",sLivre[i],sLivre[i+1],sLivre[i+2],sLivre[i+3]);
					if(i==strlen(sLivre))
						printf("////\n");
					else if(i+1==strlen(sLivre))
						printf("%c///\n",sLivre[i]);
					else if(i+2==strlen(sLivre))
						printf("%c%c//\n",sLivre[i],sLivre[i+1] );
					else if(i+3==strlen(sLivre))
						printf("%c%c%c/\n",sLivre[i],sLivre[i+1],sLivre[i+2] );
					else
						printf("%c%c%c%c\n",sLivre[i],sLivre[i+1],sLivre[i+2],sLivre[i+3]);
					adr=adr+4;
...
int bEmprunT(int iDebut,int *piFin){
	...
	char *sLivre;
	//Appel0("bEmprunT");
		bSucces=bInferieurStricT(iDebut,&iMilieu) && NLivre(iMilieu,&iSuivant) && bNoM(iSuivant,&iFin);
		...
		if (bSucces && mAsyntAnalyseR==mAsyntGenerer){//Livre répertorié et Nom inscrit
			//TdsVoir("avant emprunt");
				sLivre=sAlexChaine(iMilieu);
				bConnu=bTdsPresent(cNoM,sLivre,&iValeur);
				if (bConnu){//répertorié = connu ET disponible
...
int bReservatioN(int iDebut,int *piFin){
	//Reservation->"?" NLivre Nom	
	int bDefini1,bDefini2,bSucces,iFin,iMilieu,iSuivant,nValeur,nItem1,nItem2;
	char *sLivre,*sNom;
	//Appel0("bReservatioN");
		bSucces=bInterrogatioN(iDebut,&iMilieu) && bNLivre(iMilieu,&iSuivant) && bNoM(iSuivant,&iFin);
		*piFin=(bSucces)? iFin:iDebut;
		if (bSucces && mAsyntAnalyseR==mAsyntSemantique){
...
int bRestitutioN(int iDebut,int *piFin){
	//Restitution->">" NLivre Nom
	int bDefini1,bDefini2,bSucces,iFin,iMilieu,iSuivant,nItem1,nItem2;
	char *sNom,*sLivre;
	//Appel0("bRestitutioN");
		bSucces=bSuperieurStricT(iDebut,&iMilieu) && bNLivre(iMilieu,&iSuivant) && bNoM(iSuivant,&iFin);
		*piFin=(bSucces)? iFin:iDebut;
		if (bSucces && mAsyntAnalyseR==mAsyntSemantique){
...
int bNLivre(int iDebut,int *piFin){
	//		NLivre->LiLivre||Livre
	int bSucces=kF,iFin=0;
	
	if (bkEclaireuR){

	}else bSucces= bLiLivre(iDebut,&iFin)||bLivrE(iDebut,&iFin);
...
int bLiLivre(int iDebut,int *piFin){

	//		LiLivre->"(" DesLivre ")"
	int bSucces=kF,iFin=0,iX,iY;
	
	if (bkEclaireuR){

	}else bSucces= bParen(iDebut,&iX)&&bDesLivre(iX,&iY)&&bParen(iY,&iFin);
...
int bDesLivre(int iDebut,int *piFin){
	//		DesLivre->D1||D2
	int bSucces=kF,iFin=0;
	
	if (bkEclaireuR){

	}else bSucces= bD2(iDebut,&iFin)||bD1(iDebut,&iFin);
...
int bD1(int iDebut,int *piFin){
	//		D1->Livre	
	int bSucces=kF,iFin=0;
	
	if (bkEclaireuR){

	}else bSucces= bLivrE(iDebut,&iFin);
...
int bD2(int iDebut,int *piFin){
	//		D2->Livre DesLivre	

	int bSucces=kF,iFin=0,iX;
	
	if (bkEclaireuR){

	}else bSucces= bLivrE(iDebut,&iX)&&bDesLivre(iX,&iFin);
...
int bParen(int iDebut,int *piFin){
	int bSucces=kF;
	
	if (bkEclaireuR){

	}else bSucces= bAlexParen(iDebut);
